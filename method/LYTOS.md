...
4. **Resume is not a shortcut** — an existing branch or PR does not replace `lyt start`. If the issue is not already in `3-in-progress`, run the start phase first. If the current branch is `main`, `dev`, or `master`, stop before editing
5. **At the end of coding** — update the issue's frontmatter to `4-review`, move the file, update the BOARD.md. Promotion to `5-done` happens only after explicit validation
6. **Enrich the memory** — if a significant learning occurs, add it to the corresponding cortex file
